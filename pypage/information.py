...
from .utils import shuffle_bin_array


@nb.jit(cache=True, nogil=True, nopython=True)
def entropy(X: np.ndarray, x_bins: int, base: int = 2) -> float:
...
@nb.jit(cache=True, nogil=True, nopython=True)
def joint_entropy(
    X: np.ndarray, Y: np.ndarray, x_bins: int, y_bins: int, base: int = 2
) -> float:
    ...
    for x in np.arange(x_bins):
        for y in np.arange(y_bins):
            for z in np.arange(z_bins):
                if p_xyz[x][y][z] == 0:
                    continue
                info -= p_xyz[x][y][z] * np.log(p_xyz[x][y][z]) / np.log(base)
...
def conditional_entropy(
    ...
    for x in np.arange(x_bins):
        for y in np.arange(y_bins):
            if p_xy[x][y] == 0:
                continue
            info -= p_xy[x][y] * np.log((p_xy[x][y]) / p_y[y]) / np.log(base)
    return info
...
@nb.jit(cache=True, nogil=True, nopython=True, fastmath=True, parallel=True)
def calculate_mi_permutations(
    ...
    for idx in nb.prange(n):
        ...
        permutations[idx] = mutual_information(tmp_X, Y, x_bins, y_bins, base=base)
    return permutations
...
@nb.jit(cache=True, nogil=True, nopython=True, fastmath=True)
def measure_redundancy(
    ...
    """
    cmi = conditional_mutual_information(Y, X, Z, y_bins, x_bins, z_bins, base=base)

    mi = mutual_information(Y, Z, y_bins, z_bins, base=base)
