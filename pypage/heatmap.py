...
class Heatmap:
    ...
    def _subset_and_sort_pathways(self, max_rows: int = 50) -> (np.ndarray, np.ndarray):
        """
        ...
        """
        upregulated_mask = self.graphical_ar[:, : self.n_bins // 2].sum(
            1
        ) <= self.graphical_ar[:, self.n_bins // 2 :].sum(1)
        ...
        for i in range(self.n_bins - 2):
            sort_idxs = sorted(
                sort_idxs,
                key=lambda x: sum(sub_graphical_ar[x][i : i + 3]),
                reverse=True,
            )
    ...
    def _columnwise_heatmap(
        self, graphical_ar: np.ndarray, regulator_exp: np.ndarray, max_val: int
    ):
        self.ims = []
        if self.isreg:
    ...
    def _make_heatmap(self, max_rows, max_val, title):
        ...
        if self.isreg:
            ...
            self.fig, self.ax = plt.subplots(
                1, 1, figsize=(figure_width, figure_height)
            )
        ...
        if self.isreg:
            self.ax[0].set(
                xticks=[], yticks=np.arange(n_pathways), yticklabels=pathways
            )
    ...
    def add_gene_expression(self, genes: np.ndarray, expression: np.ndarray):
    ...
    def _calculate_regulator_expression(self):
        ...
        for i, gene in enumerate(self.regulator_names):
            if gene in self.genes:
                self.regulator_exp[i] = self.expression[
                    np.where(self.genes == gene)[0][0]
                ]
    ...
    def convert_from_to(
        ...
        if input_format in [
            ...
            self.regulator_names = np.array(
                [gene.split(".")[0] for gene in self.regulator_names]
            )
        self.regulator_names = change_accessions(
            self.regulator_names, input_format, output_format, species
        )
