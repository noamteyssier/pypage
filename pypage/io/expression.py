...
class ExpressionProfile:
    ...
    def _process_input(self, x: np.ndarray, y: np.ndarray):
        """
    ...
    def _discretize(
        self, inp_array: np.ndarray, bins: int, noise_std: Optional[float] = 0.000000001
    ):
        """
        ...
        bins_for_discr = np.interp(
            np.linspace(0, length, bins + 1), np.arange(length), np.sort(to_discr)
        )
        bins_for_discr[
            -1
        ] += 1  # otherwise numpy creates one extra bin with only 1 point
    ...
    def get_gene_subset(self, gene_subset: np.ndarray) -> np.ndarray:
        ...
        if len(self.raw_expression.shape) == 1:
            ...
            sub_expression = self.raw_expression[:, idxs]
            bin_array = np.apply_along_axis(
                lambda x: self._discretize(x, self.n_bins), 0, sub_expression
            )
