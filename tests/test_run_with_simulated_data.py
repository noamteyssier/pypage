...
def load_expression():
    df = pd.read_csv(
        "example_data/simulated_expr.csv.gz", header=0, index_col=0, compression="gzip"
    )
    exp = ExpressionProfile(df.iloc[:, 0], df.iloc[:, 1], n_bins=10)
    return exp
...
def load_ontology():
    frame = pd.read_csv(
        "example_data/simulated_df.csv.gz", header=0, index_col=0, compression="gzip"
    )
    ont = GeneSets(frame.iloc[:, 0], frame.iloc[:, 1], n_bins=6)
    return ont


def test_run(load_expression, load_ontology):
    p = PAGE(load_expression, load_ontology, n_shuffle=100, k=10, alpha=0.01)
    results, hm = p.run()

    positive = set(
        ["upregulated_" + str(i) for i in range(50)]
        + ["downregulated_" + str(i) for i in range(50)]
    )
